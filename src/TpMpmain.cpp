...
// Drop 
const char* sql_dropatable = "DROP TABLE IF EXISTS DIAGRAMS;";   
...
public:
  ...
  QgrafSQL(const std::string& dbname, bool overwritedb_) : overwritedb(overwritedb_)
  {
  ...
  bool create()
  {
    if(statusOK)
      {
        // Drop table if overwrite option selected
        if(overwritedb) sqlite3_exec(dbPtr, sql_dropatable, 0, 0, 0);
...
public:
  ...
  bool insert(const std::vector<YAML::Node>& diagrams)
  {
    ...
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(dbPtr, "INSERT OR REPLACE INTO DIAGRAMS (ID,L,P,FACTOR,PROPS,LEGS,VERTS) VALUES (?, ?, ?, ?, ?, ?, ?)", -1, &stmt, NULL);
    if (rc != SQLITE_OK)
      ...
      if(boost::filesystem::exists( yaml_path.string() ))
        {
          ...
          // 
          // TODO: switch to reading from stream prepared by hand
          //       using as delmiter end document symbols ---
          // 
          ...
          // And return number of diagrams generated by QGRAF
